...
in {
  options.player = {
    ...
    cmd = mkOption {
      type = types.str;
      default = ''uwsm app -- ghostty --class=mpd.ghostty.screen1 -e tmusic'';
  ...
  config = mkMerge [
    (mkIf (config.wayland.windowManager.hyprland.enable && cfg.enable) {
      wayland.windowManager.hyprland.settings = {
        "$player" = "${cfg.name}";
        exec-once = [
          ''[workspace 1 silent; float; size 858 559; move 640 42] ${cfg.cmd}''
        ];
        bind = [
          ''$mods, U, exec, [workspace 1 silent; float; size 858 559; move 640 42] ${cfg.cmd}''
        ];
      };
    })
